...
import { DollarSign, TrendingUp, TrendingDown, ArrowUpRight, ArrowDownRight, Plus, Trash2, Sparkles } from "lucide-react";
...
const renderCustomPieLabel = ({ name, percent, cx, cy, midAngle, innerRadius, outerRadius }: any) => {
...
export default function FinancesPage() {
  ...
  // Daily trend with income AND expenses
  const dailyTrendData = useMemo(() => {
    ...
    filtered
      ...
      .forEach(t => {
        ...
        if (t.type === "gasto") {
          expenseMap[day] = (expenseMap[day] || 0) + Math.abs(Number(t.amount));
        } else {
          incomeMap[day] = (incomeMap[day] || 0) + Number(t.amount);
        }
      });
  ...
  // Monthly comparison with income AND expenses
  const monthlyComparisonData = useMemo(() => {
    ...
    for (let i = 2; i >= 0; i--) {
      ...
      const receitas = transactions
        .filter(t => t.type === "receita" && new Date(t.transaction_date) >= d && new Date(t.transaction_date) < nextMonth)
              ...
              {dailyTrendData.length > 0 ? (
                <ResponsiveContainer width="100%" height={240}>
                  <AreaChart data={dailyTrendData}>
                    ...
                    <Legend
                      formatter={(value) => value === "receitas" ? "Receitas" : "Gastos"}
                      wrapperStyle={{ fontSize: 11, color: "hsl(240 5% 50%)" }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={monthlyComparisonData} barGap={4}>
                    ...
                    <Tooltip
                      ...
                      labelStyle={{ color: "hsl(40 20% 85%)", fontWeight: 600 }}
                      itemStyle={{ color: "hsl(40 20% 75%)" }}
                    />
                  ...
                  <ResponsiveContainer width="100%" height={280}>
                    <PieChart>
                      ...
                      <Pie
                        ...
                        labelLine={{ stroke: "hsl(240 5% 30%)", strokeWidth: 1 }}
                      >
                        {pieData.map((_, idx) => (
                    ...
                    <div key={i} className="space-y-2">
                      ...
                      <div className="w-full h-2.5 rounded-full bg-secondary/50 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${pct}%`,
                            background: `linear-gradient(90deg, ${EXPENSE_COLORS[i % EXPENSE_COLORS.length]}, ${EXPENSE_COLORS[i % EXPENSE_COLORS.length]}90)`,
                          }}
                        />
                      </div>
